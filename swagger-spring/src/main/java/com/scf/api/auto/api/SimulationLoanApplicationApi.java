...
public interface SimulationLoanApplicationApi {
	...
	@RequestMapping(value = "/simulationLoanApplication/loan/proposal/simulation", produces = {
...
//			@ApiParam(value = "Authorization token. Bearer OAuth2 token", required = true) @RequestHeader(value = "Authorization", required = true) String authorization,
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "200 OK", response = ProductsResponse.class),
			...
			@ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 403, message = "Forbidden"),
			@ApiResponse(code = 500, message = "Internal Error Server") })
	@RequestMapping(value = "/simulationLoanApplication/loan/proposal/products", produces = {
